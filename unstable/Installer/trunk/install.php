...
define('INSTALL_PACKAGE','eyeOS1505.eyepackage');
define('INSTALL_DIR','./');
...
define('eyeOS1504.eyepackage','18bb4aeb53943a4cb24dd661c34694b3');
...
function checkPreInstall(){		
	...
	$version = PHP_VERSION;
	$num = intval($version{0});
	if($num < 5){
		$errors[0] = 'Your server is currently using an unsupported version of PHP (4.x.x). Please update your PHP version to 5.x in order to update eyeOS.';
		show_faceError1($errors);
		exit(1);
	}
	if(!function_exists('gzopen')){
		$errors[0] = 'Your php installation don\'t have gzip support and eyeInstaller need it. You can install eyeOS manually</br></br> <a href=http://wiki.eyeos.org/HOWTO_Install_eyeOS_Manually>here</a>';
		show_faceError1($errors);
		exit(1);
	}
}	
...
function renameEYEROOT($newROOT){
	if(!rename(EYEROOTBASE,$newROOT)){
		$errors[0] = 'I could not rename the directory eyeOS/. Please, rename it manually to '.EYE_ROOT.'.';
		show_faceError1($errors);
		exit(1);
	}
}
...
<body id="bodyClass">
	<div id="windowContainer">
		<div id="logo"></div><div id="installText"></div>';
		if($UTF8 != true){
			echo '<div id="warning">Your PHP does not have full UTF-8 support. Please consider updating your PHP or installing the mbstring module. Without it, you will be able to install and update your eyeOS, but you may only be able to use ASCII characters on your eyeOS system.</div>'; 
		}else{
			echo '<div id="warning">The eyeOS Installer has checked your server\'s specifications and everything seems to be fine to proceed with the install. Please fill the next form with your desired root password and server\'s hostname and click on Install to proceed.</div>';
		}
		echo '
...
function checkUTF8Support(){
	if(!isMbLoaded() == true || PCREUTF8() == true){
		return true;
	}else{
		return false;
	}
